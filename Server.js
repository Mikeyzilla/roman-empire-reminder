  ...
  db.get(searchQuery, [username], async (err, user) => {
    ...
    try {
      ...
      if (match) {
        ...
        //start a countdown timer on that reminder time, going until 0 days left.
        //if the user logs in again during that time, then clear the timer and return a "timerSuccess"

        // return res.json({ timerStatus: "timerFailure", remainingDays: 0 });
        
        //indicating that the user reminded themselves of the empire during that time. 
        //else, return timerFailure. 
...
app.get('/getRemainingDays', async (req, res) => {
  const username = req.session.username; //was before {username} = req.body
